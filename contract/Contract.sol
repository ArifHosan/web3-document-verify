...
contract DocumentVerification {
    ...
    event DocumentUploaded(string indexed docId, address indexed uploader, uint256 timestamp);
    event DocumentVerified(string indexed docId, address indexed verifier, bool isVerified);
    ...
    function uploadDocument(string memory _hash) external returns (string memory) {
        documentCount++;
        string memory docId = generateUniqueId();
        documents[docId] = Document(msg.sender, _hash,  block.timestamp, false);
        emit DocumentUploaded(docId, msg.sender, block.timestamp);
        return docId;
    }

    function verifyDocument(string memory _hash, string memory _docId) external returns (string memory) {
        ...
        emit DocumentVerified(_docId, msg.sender, true);
